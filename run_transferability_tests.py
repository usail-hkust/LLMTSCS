...
import os
import time
from utils.model_transfer import test_transfer
import argparse
import wandb
...
def main(in_args):
    ...
    if in_args.dataset == 'jinan':
        ...
        traffic_file_list = ["anon_3_4_jinan_real.json", "anon_3_4_jinan_real_2000.json",
                             "anon_3_4_jinan_real_2500.json", "anon_3_4_jinan_synthetic_4000_10min.json"]
        template = "Jinan"
    ...
    if "10min" in in_args.traffic_file:
        count = 600
    ...
    # case study agents
    # use MPLight & AdvancedMPLight for 1x1 template
    # use CoLight & AdvancedCoLight for 3x2 template
    ...
    logger = wandb.init(
        ...
        name=f"{dic_traffic_env_conf['MODEL_NAME']}-{dic_traffic_env_conf['TRAFFIC_FILE_SOURCE']}-to-{dic_traffic_env_conf['TRAFFIC_FILE']}",
