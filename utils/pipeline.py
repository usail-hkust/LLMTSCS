...
from .my_utils import dump_json
import json
import shutil
import os
import time
...
class Pipeline:
    ...
    def run(self, round, multi_process=False):
        ...
        for cnt_round in range(self.dic_traffic_env_conf["NUM_ROUNDS"]):
            ...
            results, state_action_log = model_test.test(self.dic_path["PATH_TO_MODEL"], self.dic_path["PATH_TO_DATA"], cnt_round,
                                                        self.dic_traffic_env_conf["RUN_COUNTS"], self.dic_traffic_env_conf, logger)
