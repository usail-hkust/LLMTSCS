...
from .my_utils import get_state_detail, dump_json
...
def test_transfer(data_dir, dic_traffic_env_conf, dic_agent_conf):
    ...
    traffic_file = dic_traffic_env_conf['TRAFFIC_FILE'].split('.')[0]
    round_num = 99
    ...
    for i in range(len(inter_agents)):
        inter_agents[i].load_network(f'round_{round_num}_inter_0', file_path=f"./case_study_models/{dic_agent_conf['MODEL_NAME']}/{traffic_file_source}/")
    ...
    plt.hist(all_queues, bins=20, alpha=0.8, color='#779BFF')
    plt.xticks(np.arange(0, 100, step=10), fontsize=40)
    plt.yticks(np.arange(0, 500, step=25), fontsize=40)
    plt.xlabel("Queue Length", fontsize=40, labelpad=18)
    plt.ylabel("Occurrence", fontsize=40, labelpad=20)
    plt.xlim([0, 100])

    plt.savefig(f'./figs/queue_lengths_{traffic_file}.svg', bbox_inches='tight')
    plt.show()
